...
async def tag_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=CHANNEL_ID, text=f"✅ Bot connected. Starting URL posting for: {keyword}")
    except Exception as e:
...
# ---------------- Async main ----------------
async def main():
    WEBHOOK_URL = f"https://tpost-szdp.onrender.com/{BOT_TOKEN}"  # your Render app URL

    # Properly await async webhook calls
    await bot_app.bot.delete_webhook()
    await bot_app.bot.set_webhook(WEBHOOK_URL)
    print("✅ Webhook set:", WEBHOOK_URL)

    # Start FastAPI + telegram bot updates queue
